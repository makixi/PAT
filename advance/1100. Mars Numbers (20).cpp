...
int main(){
    ...
    for(int i=0; i<N; i++){
        ...
        if(mess[0]>='0'&&mess[0]<='9') {
            for(int j=0; j<mess.size(); j++)number=number*10+mess[j]-'0';
            if(number<13)cout<<fire[number]<<endl;
            else {
            ...
            for(int k=0; k<mess.size(); k=k+4){
                ...
                for(int l=0; l<13; l++){
                    if(three==fire[l]) number=number+l;
                    if(three==jinzhi[l])number=number+(l+1)*13;
                }
                three="";
            }
            cout<<number<<endl;
